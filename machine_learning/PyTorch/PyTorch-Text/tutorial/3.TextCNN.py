...
input_batch = torch.tensor(inputs).long()
target_batch = torch.tensor(targets).long()
dtype = torch.float
...
class TextCNN(nn.Module):
    ...
    def __init__(self):
        ...
        # self.W = nn.Parameter(torch.empty(vocab_size, embedding_size)
        #                       .uniform_(-1, 1), requires_grad=True).type(dtype)
        self.W = nn.Embedding(vocab_size, embedding_size)
        # self.Weight = nn.Parameter(torch.empty(self.num_filters_total, num_classes)
        #                            .uniform_(-1, 1), requires_grad=True).type(dtype)
        # self.Bias = nn.Parameter(0.1 * torch.ones([num_classes]), requires_grad=True).type(dtype)
        self.linear = nn.Linear(self.num_filters_total, num_filters)
    ...
    def forward(self, X):
        ...
        # return model
        return self.linear(h_pool_flat)
...
test_batch = torch.tensor(tests).long()
